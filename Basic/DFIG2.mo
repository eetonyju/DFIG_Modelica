...
/*
    ...
    (real(Upc_p))^2+(imag(Upc_p))^2=U_sp^2;//4
      I_rnet_n = 0 + 0 * j;//equation17
    U_r_p/s_p=I_rnet_p*(Rr/s_p+j*Xr)+I_snet_p*(Rs+j*Xs)+Upc_p;//5
    (I_rnet_p-I_snet_p)*j*Xm-I_snet_p*(Rs+j*Xs)-Upc_p=0+0*j;//6
    Q_DFIG_p=3*(Q_sp+Q_gp);//13
    */
        Upc_a = Upc_0 + Upc_p + Upc_n;
        Upc_b = Upc_0 + a ^ 2 * Upc_p + a * Upc_n;
        Upc_c = Upc_0 + a * Upc_p + a ^ 2 * Upc_n;
