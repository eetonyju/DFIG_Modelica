...
model DFIG_Kamh_B13
        ...
        parameter Real Qs_sp_p = 0.0;
          parameter Real Pg_sp_p = -s_p * P_DFIG_p;
        // parameter Real Q_sp= 0;
        // parameter Real Q_gp= 0;
      ...
      //14
          U_r_p / s_p = I_rnet_p * (Rr / s_p + j * Xr) + I_snet_p * (Rs + j * Xs) + Upc_p;
      //5
          (I_rnet_p - I_snet_p) * j * Xm - I_snet_p * (Rs + j * Xs) - Upc_p = 0 + 0 * j;
      //6
        3 * real(Upc_p * conj(I_snet_p)) = P_DFIG_p - Pg_sp_p;
          3 * real(Upc_p * conj(I_gnet_p)) = Pg_sp_p;
